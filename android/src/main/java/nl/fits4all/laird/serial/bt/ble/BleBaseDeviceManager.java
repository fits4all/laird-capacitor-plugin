package nl.fits4all.laird.serial.bt.ble;
...
public abstract class BleBaseDeviceManager extends BluetoothGattCallback {
    ...
    public BleBaseDeviceManager(Activity activity) {
        if (activity == null) throw new NullPointerException("Activity object passed is null!");
    ...
    public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
        ...
        switch (status) {
            case BluetoothGatt.GATT_SUCCESS:
                mValueRSSI = rssi + "";
                Log.d(TAG, "onReadRemoteRssi: GATT_SUCCESS");
                break;
